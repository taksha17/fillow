...
function humanLine(reporter, record) {
  ...
  if (type === "run.start") {
    writeLine(`\n${chalk.cyan(prefix)} ${label || "starting"}`);
    ...
    writeLine(`\n${chalk.blue(prefix)} ${label}`);
  } else if (type === "phase.complete") {
    writeLine(`${chalk.blue(prefix)} ${label} complete`);
    ...
    writeLine(`  ${chalk.yellow("start")} ${data.item || label}`);
...
export function createDashboard(agent = "fillow") {
  const agents = {
    discover: { status: "idle", startTime: null, endTime: null, jobs: 0, errors: 0 },
    evaluate: { status: "idle", startTime: null, endTime: null, jobs: 0, errors: 0 },
    apply: { status: "idle", startTime: null, endTime: null, jobs: 0, errors: 0 },
  ...
  function drawBox(x, y, width, height, title = '') {
    ...
    output += `${topLeft}${horizontal.repeat(width - 2)}${topRight}\n`;

    if (title) {
      const padding = Math.floor((width - title.length - 2) / 2);
      output += `${vertical}${' '.repeat(padding)}${title}${' '.repeat(padding)}${title.length % 2 === 0 ? '' : ' '}${vertical}\n`;
    }
    ...
    output += `${bottomLeft}${horizontal.repeat(width - 2)}${bottomRight}\n`;
  ...
  function drawAgentBox(agent, data, x, y) {
    ...
    let content = `
${colors(agent)} Agent: ${agent.padEnd(8)}${statusColors(data.status)}Status: ${data.status}
${colors(agent)} Jobs: ${String(data.jobs).padEnd(6)} Errors: ${String(data.errors).padEnd(4)}
${colors(agent)} Started: ${data.startTime ? new Date(data.startTime).toLocaleTimeString() : 'Not started'}
${colors(agent)} Ended: ${data.endTime ? new Date(data.endTime).toLocaleTimeString() : 'Not ended'}
`;

    drawBox(x, y, boxWidth, 6, agent.toUpperCase());
  ...
  function logAgent(agent, message, data = {}) {
    console.log(`${chalk.gray(`[${new Date().toLocaleTimeString()}]`)} ${chalk.cyan(agent)} ${message}`);
  }
