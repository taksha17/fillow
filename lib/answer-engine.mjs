...
import { parseCandidateLocation } from "./location.mjs";

const FREE_TEXT_MARKERS = ["describe", "explain", "tell us", "write", "essay", "paragraph"];

/**
 * Generic screening: prior employment / friends-family at the hiring company → No.
 * Company name in the question is irrelevant — do not store per-company answers in profile/qa-bank.
 */
export function isGenericCompanyAssociationNo(question) {
  const q = String(question || "").toLowerCase().replace(/\s+/g, " ").trim();
  if (!q) return false;

  // Current / most-recent employer & title are identity facts, not Yes/No association.
  if (
    /\b(current|most recent|previous)\b.{0,20}\b(employer|job title|company)\b/.test(q)
    || /\bwho is your (current|previous)\b/.test(q)
    || /\bcurrent company\b/.test(q)
    || /\bcurrent title\b/.test(q)
  ) {
    return false;
  }
  // Work-auth / sponsorship must stay sponsorship-aware.
  if (/\bsponsor|\bvisa\b|authorized to work|work authorization|lawfully work/.test(q)) {
    return false;
  }

  const priorEmployment =
    /\bpreviously (been )?employ/.test(q)
    || /\bprevious(ly)? (been an? )?employee\b/.test(q)
    || /\bformer employee\b/.test(q)
    || /\bever (been )?employ/.test(q)
    || /\bever work(ed)? (for|at|with)\b/.test(q)
    || /\bworked for (this |the )?(company|firm|organization)\b/.test(q)
    || /\bemployed by\b/.test(q)
    || /\bhistory with\b/.test(q);

  const friendsFamily =
    /\b(related|relative|relatives|family|friend|friends|personal relationship|employee-owner)\b/.test(q)
    && /\b(employ|employed|employer|employee|work|working|worked)\b/.test(q);

  const knowAnyone =
    /\bknow anyone\b/.test(q) && /\b(work|employ)/.test(q);

  const govPartners =
    (/\bgovernment agency\b/.test(q) && /\b(family|friend|relative|partner)\b/.test(q))
    || /\bcommercial partners?\b/.test(q);

  return Boolean(priorEmployment || friendsFamily || knowAnyone || govPartners);
}
...
export function profileLockedAnswer(question, candidate) {
  ...
  // Torc/export-control "applicable country" / "different country" follow-ups are NOT
  // country_of_residence — "reside" also matches inside "resident"/"residency".
  if (
    /applicable country|indicate the applicable country|checked either|work visa there|different country/i.test(
      q
    )
  ) {
    return null;
  ...
  if (
    ...
    || /\bcurrently reside\b/.test(q)
    || (/\bcountry\b/.test(q) && /\b(reside|resides|residing|residency)\b/.test(q))
...
export function heuristicAnswer(question, candidate, job = {}, prefs = {}, options = []) {
  ...
  // Generic "ever worked here / friends-family here" → No (any company name).
  // Runs before qa-bank so we never need per-company bank/profile rows for these.
  if (isGenericCompanyAssociationNo(question)) {
  ...
  // Exception: multi-select immigration/export follow-ups use the bank/prefs, not Yes/No sponsorship.
  const isExportFollowUp =
    /export control|citizen, national, or resident of any|checked any of the boxes above|applicable country|checked either|work visa there/i.test(
      q
    );
  // Skip bank for export-control + sponsorship — those need options/profile facts, not a stale bank string.
  const banked =
    isExportFollowUp || /\bsponsor|source of (?:your )?right to work/.test(q)
      ? null
      : qaBankAnswer(question);
  if (banked) return pickApiOption(options, banked) || banked;

  if (isExportFollowUp) {
    if (/applicable country|checked either|indicate the applicable/i.test(q)) {
      return (
        prefs.export_control_country_detail
        || candidate.passport_country
        || ""
      );
    }
    if (/checked any of the boxes above|additional information regarding your immigration/i.test(q)) {
      return (
        pickApiOption(options, prefs.export_control_immigration || "")
        || prefs.export_control_immigration
        || ""
      );
    }
    if (/export control|citizen, national, or resident of any/i.test(q)) {
      return (
        pickApiOption(options, prefs.export_control_countries || "")
        || options.find((o) => /none of the above|none\/not applicable|\bn\/a\b/i.test(String(o)))
        || prefs.export_control_countries
        || "None/Not applicable"
      );
    }
  }
  ...
  // prior employment / friends-family covered by isGenericCompanyAssociationNo (above)
  if (
  ...
  if (q.includes("sponsor") || (q.includes("immigration") && !/checked any of the boxes|additional information regarding your immigration/i.test(q))) {
  ...
  if (
    (q.includes("office") && (q.includes("three days") || q.includes("3 days") || /3\s*days/.test(q)))
    || (q.includes("work from") && q.includes("office"))
  ) {
    return pickApiOption(options, "Yes") || "Yes";
  }
  ...
  if (
    ...
    || q.includes("authorized to lawfully work")
    || q.includes("lawfully work")
    || q.includes("legally authorized")
    || q.includes("legally authorised")
  ...
  if (
    ...
    const loc = candidate.location || "Austin, TX, USA";
    return `${loc}. Yes — willing to relocate for this role.`;
  }
  if (
    (q.includes("currently located") || q.includes("where are you located") || q.includes("current location"))
    && !q.includes("relocat")
  ) {
    const loc = candidate.location || "Austin, TX, USA";
    const { city, state } = parseCandidateLocation(loc);
    return pickApiOption(options, loc)
      || pickApiOption(options, city)
      || pickApiOption(options, state)
      || loc;
  }
  if (q.includes("worldquant university") || (q.includes("enrolled") && q.includes("university") && q.includes("past 12"))) {
    return pickApiOption(options, "No") || "No";
  }
  if (
  ...
  if (q.includes("bound by any agreements") || q.includes("restrict your ability to work") || q.includes("restrict your ability to accept") || q.includes("restrict any work")) {
    return pickApiOption(options, "No") || "No";
  }
  if (q.includes("export control") || (q.includes("citizen, national, or resident") && q.includes("countries"))) {
    return pickApiOption(options, prefs.export_control_countries || "None/Not applicable")
      || prefs.export_control_countries
      || "None/Not applicable";
  }
  if (
    q.includes("additional information regarding your immigration")
    || (q.includes("checked any of the boxes") && (q.includes("none") || q.includes("immigration")))
  ) {
    return pickApiOption(options, prefs.export_control_immigration || "")
      || prefs.export_control_immigration
      || "";
  }
  if ((q.includes("salary") || q.includes("compensation")) && (q.includes("range") || q.includes("expect") || q.includes("target") || q.includes("desire"))) {
    return pickApiOption(options, prefs.salary_range || "") || prefs.salary_range || "";
  }
  if (q.includes("additional information you would like us to consider") || (q.includes("evaluating your application") && q.includes("provide"))) {
    return (prefs.claims?.additional_info || "").trim() || (prefs.claims?.general || "").trim() || "";
  }
  ...
  if (/^location\*?$/.test(q.trim()) || (q.includes("location") && !q.includes("relocat") && !q.includes("work from") && q.length < 24)) {
    return candidate.greenhouse_location || candidate.location || "";
  }
...
export function applyProfileLocks(answers, candidate, optionsByQuestion = {}) {
  ...
  for (const q of Object.keys(out)) {
    // Never overwrite Torc export-control country follow-ups with country_of_residence.
    if (
      /applicable country|indicate the applicable country|checked either|work visa there|different country/i.test(
        q
      )
