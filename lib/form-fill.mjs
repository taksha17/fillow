...
async function applyAshby(page, job, ctx) {
  ...
  for (const target of candidates) {
    try {
      ...
      const body = await visibleBody(page);
      const formCount = await page.locator("#_systemfield_name, #_systemfield_email, #_systemfield_resume").count();
      const hard404 =
        title === "not found"
        || title.startsWith("page not found")
        || (/^page not found\b/.test(body.trim()) && !formCount)
        || (body.includes("page not found") && !formCount);
      if (hard404) continue;
      try {
        await page.waitForSelector(
          "#_systemfield_name, #_systemfield_email, #_systemfield_resume, input[type='email'], button:has-text('Apply')",
          { timeout: 12000 }
        );
        opened = true;
        break;
      } catch {
        if (await page.locator("input, textarea, button").count()) {
          opened = true;
          break;
        }
      }
  ...
  if (questions.length) {
    ...
    if (needLlm.length && ctx.llmChat) {
      ...
      answers = { ...answers, ...extra };
      await applyAnswers(page, needLlm, answers, ctx.coverText, ctx.humanize);
    }
  }
