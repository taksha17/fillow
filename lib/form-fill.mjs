...
async function applyGreenhouse(page, job, ctx) {
  ...
  const optionsByQuestion = Object.fromEntries(apiQuestions.filter((q) => q.options.length).map((q) => [q.label, q.options]));
  const labels = [...new Set(apiQuestions.map((q) => q.label))];
  let answers = {};
  if (labels.length) {
    console.log(`  Greenhouse questions: ${labels.length}`);
    ...
    for (const q of apiQuestions) {
      ...
      let filled = false;
      if (selectType) filled = await reactSelect(root, q.fieldId, option, { humanize: ctx.humanize });
      if (!filled) filled = await fillGreenhouseField(root, q.fieldId, option, ctx.humanize);
      if (!filled && (option === "Yes" || option === "No")) {
        filled = await reactSelect(root, q.fieldId, option, { humanize: ctx.humanize });
      }
    }
  }
  ...
  await answerByLabelFallbacks(root, ctx.cfg.candidate, ctx.cfg.answer_preferences, ctx.humanize);
  ...
  const leftover = discovered.filter((d) => {
    if (IDENTITY_FIELD_IDS.has(d.forId)) return false;
    if (isEeoLabel(d.text) && !d.options.length) return true;
    return !answers[d.text];
  });
  if (leftover.length) {
    const moreOpts = { ...optionsByQuestion };
    for (const item of leftover) {
      if (item.options?.length) moreOpts[item.text] = item.options;
    }
    const extra = await answerQuestions({
      questions: leftover.map((d) => d.text),
      candidate: ctx.cfg.candidate,
      job,
      optionsByQuestion: moreOpts,
      prefs: ctx.cfg.answer_preferences,
      llmChat: ctx.llmChat,
    });
    answers = { ...answers, ...extra };
    await fillDiscoveredFields(root, leftover, ctx, answers);
  }
