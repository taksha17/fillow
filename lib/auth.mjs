...
export async function checkNim(cfg) {
  ...
  if (!key) {
    return entry("NVIDIA NIM", true, "missing", "NVIDIA_API_KEY not set", "add it to .env — required for answers + score verify");
  }
  try {
    const res = await probe("https://integrate.api.nvidia.com/v1/models", {
      headers: { Authorization: `Bearer ${key}` },
    });
    if (res.status === 401) return entry("NVIDIA NIM", true, "fail", "401 Unauthorized — key rejected", "rotate the key; it may have been revoked");
    if (!res.ok) return entry("NVIDIA NIM", true, "warn", `HTTP ${res.status}`, "service reachable but key state unclear");
    return entry("NVIDIA NIM", true, "ok", `${cfg.ai?.model || "nemotron"} reachable`);
  } catch (err) {
    return entry("NVIDIA NIM", true, "fail", err.message, "network error reaching NIM");
  }
}
...
export async function checkGroq(cfg) {
  ...
  if (!key) return entry("Groq (LLM fallback)", false, "off", "not set", "optional — used when NIM is unavailable");
  try {
    const res = await probe("https://api.groq.com/openai/v1/models", { headers: { Authorization: `Bearer ${key}` } });
    if (res.status === 401) return entry("Groq (LLM fallback)", false, "fail", "401 Unauthorized", "rotate GROQ_API_KEY");
    if (!res.ok) return entry("Groq (LLM fallback)", false, "warn", `HTTP ${res.status}`);
    return entry("Groq (LLM fallback)", false, "ok", "models reachable");
  } catch (err) {
    return entry("Groq (LLM fallback)", false, "fail", err.message);
  }
}
