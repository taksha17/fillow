...
export function urlFromNotes(notes) {
...
export function readTracker(path = PATHS.applications) {
  ...
  return readFileSync(path, "utf8")
    ...
    .map(parseRow)
    .filter(Boolean)
    .map((row) => ({ ...row, url: urlFromNotes(row.notes) }));
}
...
export function normalizeUrl(url) {
  ...
  try {
    const u = new URL(raw);
    // Drop search/hash so ?query=… noise never becomes the identity.
...
/** Stable MyGreenhouse identity: board + greenhouse job id. */
export function mghJobKey(urlOrId) {
  const s = String(urlOrId || "");
  const m =
    s.match(/my\.greenhouse\.io\/jobs\/([^/?#]+)\/(\d+)/i)
    || s.match(/\bmgh:([^:]+):(\d+)\b/i);
  if (!m) return "";
  return `mgh:${m[1].toLowerCase()}:${m[2]}`;
}

export function hasUrl(rows, url) {
  const key = normalizeUrl(url);
  const mgh = mghJobKey(url);
  if (!key && !mgh) return false;
  return rows.some((row) => {
    const notes = String(row.notes || "");
    if (mgh && mghJobKey(notes) === mgh) return true;
    if (key && (normalizeUrl(notes).includes(key) || notes.toLowerCase().includes(key))) return true;
    return false;
  });
}

export function hasCompanyRole(rows, company, role) {
  const c = String(company || "").toLowerCase().trim();
  const r = String(role || "").toLowerCase().trim();
  if (!c || !r) return false;
  return rows.some(
    (row) =>
      String(row.company || "").toLowerCase().trim() === c
      && String(row.role || "").toLowerCase().trim() === r
      && /applied|submitted|review|dry_run/i.test(String(row.status || ""))
  );
}
...
export function appendApplication(entry, path = PATHS.applications) {
  ...
  if (url && /my\.greenhouse\.io\/jobs\/search/i.test(url)) {
    // Never store the MGH search page as the application identity.
  }
  if (url && hasUrl(rows, url)) {
    return { skipped: true, reason: "duplicate_url", rows };
  }
  if (hasCompanyRole(rows, entry.company, entry.role || entry.title)) {
    return { skipped: true, reason: "duplicate_company_role", rows };
  }
