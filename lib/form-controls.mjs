...
export function idSel(fieldId) {
  const raw = String(fieldId || "").replace(/"/g, "");
  if (!raw) return "";
  const stripped = raw.replace(/\[\]$/, "");
  // Greenhouse multi-selects often keep [] in the DOM id (Datadog cities/languages).
  if (stripped !== raw) return `[id="${raw}"], [id="${stripped}"]`;
  return `[id="${raw}"]`;
}
...
export function isAgreeLabel(text) {
  ...
  return (
    /\bi agree\b/.test(q)
    || (q.includes("by selecting") && q.includes("agree"))
    || /\bterms and conditions\b/.test(q)
    || /privacy policy/.test(q)
    || /\backnowledge\b/.test(q) && /(read|understood|privacy|policy|terms)/.test(q)
  );
}
...
/** Stripe/Greenhouse country lists often use "US" while profile.yaml says "United States". */
export function countryAliases(want) {
  const w = String(want || "").trim().toLowerCase().replace(/\./g, "");
  if (!w) return [];
  const aliases = new Set([w, String(want || "").trim().toLowerCase()]);
  if (
    w === "us"
    || w === "usa"
    || w === "united states"
    ...
    for (const a of ["us", "usa", "united states", "united states of america", "u.s.", "u.s.a."]) {
      aliases.add(a);
    }
  }
  return [...aliases];
...
export function scoreOption(text, want) {
  ...
  if (!low || !w) return -1;
  if (low === w) return 100;
  if (countryAliases(want).includes(low)) return 98;
  ...
  // Short tokens like "Other" false-match inside longer labels (e.g. brOthers).
  if (w.length <= 5) {
    const re = new RegExp(`(?:^|[^a-z0-9])${w.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?:[^a-z0-9]|$)`);
    if (re.test(low)) return 70;
    return -1;
  }
...
export function pickApiOption(values, want) {
  ...
  for (const alias of countryAliases(want)) {
    const hit = labels.find((lab) => lab.toLowerCase() === alias);
    if (hit) return hit;
  }
  if (w === "yes") {
  ...
  if (w === "i agree" || w === "acknowledge" || w.startsWith("i acknowledge")) {
    const agree = labels.find((lab) => {
      const l = lab.toLowerCase();
      return l.includes("agree") || l.includes("acknowledge") || l.includes("understood") || l.includes("accept");
    });
    if (agree) return agree;
  }
  const wNorm = w.replace(/[–—]/g, "-").replace(/\s+/g, "");
  // Prefer exact-ish / shortest substring hits — "LinkedIn" must beat "LinkedIn Ad (India)".
  let subBest = null;
  let subBestLen = Infinity;
  for (const lab of labels) {
    const low = lab.toLowerCase();
    const n = low.replace(/[–—]/g, "-").replace(/\s+/g, "");
    if (n === wNorm) return lab;
    if ((w === "male" || w === "man" || w === "men") && /female|woman|women/.test(low)) continue;
    if (w.length <= 5 && low !== w && !countryAliases(want).includes(low)) continue;
    // Never use unanchored substring for short wants — "US" sits inside "Australia".
    if (w.length >= 4 && (low.includes(w) || (lab.length >= 4 && w.includes(low)))) {
      if (low.length < subBestLen) {
        subBest = lab;
        subBestLen = low.length;
      }
    }
  }
...
export function schoolNameTries(candidate = {}, { greenhouse = false } = {}) {
  const tries = [];
  const seen = new Set();
  ...
  // Greenhouse school catalogs rarely include every university; try one primary
  // name (with The-/stripped variants) then Other — avoid looping aliases.
  if (greenhouse) {
    const primary = [];
    const pSeen = new Set();
    const pPush = (name) => {
      const t = String(name || "").trim();
      if (!t) return;
      uniqPush(primary, pSeen, t);
      uniqPush(primary, pSeen, t.replace(/^the\s+/i, "").trim());
    };
    pPush(candidate.school);
    uniqPush(primary, pSeen, "Other");
    return primary;
  }
...
export function degreeNameTries(candidate = {}) {
  ...
  // Greenhouse boards use short canonical labels — try those before long profile strings
  // so type-to-filter does not leave uncommitted input text that looks "filled".
  if (/master/i.test(d)) {
    for (const x of ["Master's Degree", "Master's", "Masters", "Master of Science", "M.S.", "MS", "MA", "M.A."]) {
      uniqPush(tries, seen, x);
    }
  }
  if (/bachelor/i.test(d)) {
    for (const x of ["Bachelor's Degree", "Bachelor's", "Bachelors", "Bachelor of Science", "B.S.", "BS", "BA", "B.A."]) {
      uniqPush(tries, seen, x);
    }
  }
...
export function pickVeteranOption(options = [], want = "No") {
  ...
  if (no) {
    ...
    // Greenhouse OFCCP widgets often expose no options until opened; type the OFCCP string.
    if (hit) return hit;
    return labels.length ? null : "I am not a protected veteran";
  }
  const hit = labels.find((lab) => {
    const l = lab.toLowerCase();
  ...
  if (hit) return hit;
  return labels.length ? pickApiOption(labels, want) : "I am a protected veteran";
}
