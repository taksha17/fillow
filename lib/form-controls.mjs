...
export function pickApiOption(values, want) {
  ...
  for (const lab of labels) {
    ...
    if (w.length >= 4 && (lab.toLowerCase().includes(w) || (lab.length >= 4 && w.includes(lab.toLowerCase())))) {
      if ((w === "male" || w === "man" || w === "men") && /female|woman|women/.test(lab.toLowerCase())) continue;
      if (w.length <= 5 && lab.toLowerCase() !== w && !countryAliases(want).includes(lab.toLowerCase())) continue;
      return lab;
    }
  }
  let best = null;
