...
export const FIT_STEPS = [
  { fs: 10.5, lh: 1.12, drop: 0, maxBullets: 4, maxProjects: 3, maxRoles: 4 },
  { fs: 10, lh: 1.1, drop: 0, maxBullets: 3, maxProjects: 3, maxRoles: 4 },
  { fs: 9.5, lh: 1.08, drop: 1, maxBullets: 3, maxProjects: 2, maxRoles: 3 },
  { fs: 9, lh: 1.06, drop: 1, maxBullets: 3, maxProjects: 2, maxRoles: 3 },
  { fs: 8.75, lh: 1.05, drop: 2, maxBullets: 2, maxProjects: 2, maxRoles: 3 },
];
...
async function tailorModel(cfg, job, extra, step = FIT_STEPS[0]) {
  return tailorResumeForJob(baseResumeFromConfig(cfg, extra), job, {
    dropPerRole: step.drop ?? 0,
    maxBulletsPerRole: step.maxBullets ?? 4,
    maxProjects: step.maxProjects ?? 3,
    maxRoles: step.maxRoles ?? 4,
  });
}

async function prepareTailoredModel(cfg, job, extra = {}) {
  await ensureJobDescription(job);
  const model = tailorResumeForJob(baseResumeFromConfig(cfg, extra), job, {
    dropPerRole: 0,
    maxBulletsPerRole: 4,
    maxProjects: 3,
    maxRoles: 4,
  });
  // Rewrite every role/project that still has bullets (JD-selected subset).
  for (const role of model.experience || []) {
    if (role.bullets?.length) {
      role.bullets = await rewriteBulletsWithLlm(cfg, job, role.bullets);
    }
  }
  for (const project of model.projects || []) {
    if (project.bullets?.length) {
      project.bullets = await rewriteBulletsWithLlm(cfg, job, project.bullets);
    }
  }
  return model;
}
...
export async function renderTailoredResume(cfg, job, extra = {}, step = FIT_STEPS[0], prepared) {
  const model = prepared
    ? tailorResumeForJob(prepared, job, {
        dropPerRole: step.drop ?? 0,
        maxBulletsPerRole: step.maxBullets ?? 4,
        maxProjects: step.maxProjects ?? 3,
        maxRoles: step.maxRoles ?? 4,
      })
    : await tailorModel(cfg, job, extra, step);
...
export async function generateSinglePdf(cfg, job, { extra = {}, force = false, browser } = {}) {
  ...
  if (!force && existsSync(pdfPath) && existsSync(mdPath)) return pdfPath;
  if (!force && existsSync(pdfPath)) {
    const prepared = await prepareTailoredModel(cfg, job, extra);
    writeFileSync(mdPath, `${renderResumeMarkdown(cfg.candidate, prepared)}\n`, "utf8");
    return pdfPath;
  }
  ...
  try {
    ...
    for (const step of FIT_STEPS) {
      const { html, model } = await renderTailoredResume(cfg, job, extra, step, prepared);
      lastHtml = html;
      lastModel = model;
    ...
    writeFileSync(mdPath, `${renderResumeMarkdown(cfg.candidate, lastModel)}\n`, "utf8");
    return pdfPath;
  } finally {
...
export function renderResumeMarkdown(candidate, model) {
  const name = String(
    candidate?.full_name || `${candidate?.first_name || ""} ${candidate?.last_name || ""}`.trim() || "Resume"
  );
  const lines = [`# ${name}`, ""];
  const contact = [candidate?.email, candidate?.location, candidate?.linkedin, candidate?.github]
    .filter(Boolean)
    .map((v) => displayLink(v));
  if (contact.length) lines.push(contact.join(" · "), "");

  // Match Jake HTML section order: Education → Experience → Projects → Skills
  if (model?.education?.length) {
    lines.push("## Education", "");
    for (const e of model.education) {
      lines.push(`### ${e.school}${e.location ? ` — ${e.location}` : ""}`, "");
      lines.push(`*${e.degree}${e.dates ? ` | ${e.dates}` : ""}*`, "");
    }
  }
  ...
  if (model?.projects?.length) {
    ...
    for (const p of model.projects) {
      lines.push(`### ${p.name}${p.stack ? ` | ${p.stack}` : ""}${p.dates ? ` | ${p.dates}` : ""}`, "");
      for (const b of p.bullets || []) lines.push(`- ${b}`);
      lines.push("");
    }
  }

  if (model?.skills && Object.keys(model.skills).length) {
    lines.push("## Technical Skills", "");
    for (const [key, label] of Object.entries(SKILL_LABELS)) {
      const items = (model.skills[key] || []).filter(Boolean);
