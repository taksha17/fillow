...
import { readJobs } from "./jobs-tsv.mjs";
import { metricsFromRows } from "./tracker.mjs";
...
VALUES (${Number(row.num) || "NULL"}, ${sqlStr(row.date)}, ${sqlStr(row.company)}, ${sqlStr(row.role)}, ${sqlStr(row.score)}, ${sqlStr(row.status)}, ${sqlStr(row.pdf)}, ${sqlStr(row.report)}, ${sqlStr(row.notes)}, ${sqlStr(row.url)}, ${sqlStr(payload.captured_at)})
