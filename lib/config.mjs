...
export function loadConfig() {
  ...
  const applyEngineRaw = String(process.env.APPLY_ENGINE || runtimeYaml.apply_engine || "playwright").toLowerCase();
  const applyEngine = ["playwright", "bsk"].includes(applyEngineRaw) ? applyEngineRaw : "playwright";
  const discoverEngineRaw = String(process.env.DISCOVER_ENGINE || runtimeYaml.discover_engine || "api").toLowerCase();
  const discoverEngine = ["api", "bsk"].includes(discoverEngineRaw) ? discoverEngineRaw : "api";
  ...
  return {
    ...
    runtime: {
      ...
      // Agent 3 / MyGreenhouse: call LLM for fields heuristics cannot answer (default on).
      llm_fill_missing: runtimeYaml.llm_fill_missing ?? true,
    },
    ai: {
      provider: "nvidia_nim",
    ...
    gmail: {
      ...
      otp_regex: raw.gmail?.otp_regex || "^[0-9]{4,8}$|^[0-9A-Za-z]{8}$",
    },
