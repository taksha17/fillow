...
/** Patch status (and optional notes) for a job by source+external_id. */
export function updateJobStatus(externalId, status, { source = "mygreenhouse", notes } = {}, path = PATHS.jobsTsv) {
  const jobs = readJobs(path);
  const needle = String(externalId || "").toLowerCase();
  let updated = 0;
  for (const job of jobs) {
    if (String(job.source || "").toLowerCase() !== String(source).toLowerCase()) continue;
    if (String(job.external_id || "").toLowerCase() !== needle) continue;
    job.status = status;
    if (notes != null) job.notes = String(notes).slice(0, 240);
    updated += 1;
  }
  if (updated) writeJobs(jobs, path);
  return updated;
}
