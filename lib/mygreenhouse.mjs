...
async function answerAndFill(sessionId, fields, job, cfg, llmChat) {
  ...
  // Watch mode: skip NIM — long NIM timeouts idle-out the Agent Window (~300s).
  // Heuristics + profile locks + qa-bank are enough for Yes/No and common fields.
  const useLlm = typeof llmChat === "function" && !isWatchMode();
  console.log(
    `  MGH empty fields: ${empty.length}; answering via profile/bank${useLlm ? "/NIM" : " (watch: heuristics, no NIM)"}`
  );
...
export async function applyMyGreenhouseJob(sessionId, job, cfg, { dryRun = true, resumePath = null, llmChat = null, oneShot = false } = {}) {
      ...
      const apps = await evaluate(
        ...
        `(function(){
          ...
          for (let i=0;i<lines.length;i++) {
            ...
            if (coNorm && (low.includes(company) || n.includes(coNorm))) { idx=i; break; }
            if (title && title.length>8 && low.includes(title.slice(0,24))) { idx=i; break; }
          }
          const ctx=idx>=0 ? lines.slice(Math.max(0,idx-2), idx+5) : [];
          const blob=ctx.join(' | ');
          return { found: idx>=0, applied: /applied/i.test(blob), justNow: /just now|minutes? ago|seconds? ago/i.test(blob), ctx };
        })()`
      );
      console.log(`  MGH oneShot apps check: ${JSON.stringify(apps)}`);
      if (apps?.found && apps?.applied) {
        return {
          status: "applied",
