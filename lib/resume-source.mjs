...
export function baseResumeFromConfig(cfg, extra = {}) {
  ...
  if (!experience.length && candidate.current_company) {
    // only create an experience entry if there are actual claim lines;
    // otherwise fall back to the fallback skills below
...
export function tailorResumeForJob(base, job, { dropPerRole = 0 } = {}) {
  ...
  const experience = (base.experience || []).map((role) => {
    ...
    // keep only bullets that have actual relevance (score > 0), unless none do then keep the top N
    const hasRelevant = scored.some((x) => x.s > 0);
    const keep = hasRelevant ? Math.max(1, scored.length - dropPerRole) : Math.max(0, scored.length - dropPerRole);
    const picked = scored.slice(0, keep).sort((a, b) => a.i - b.i).map((x) => x.b);
    // if we kept 0 bullets but had some originally, keep the single most-relevant (even if score 0)
    if (picked.length === 0 && scored.length > 0) {
      picked.push(scored.sort((a, b) => a.i - b.i)[0].b);
    }
...
export async function rewriteBulletsWithLlm(cfg, job, bullets) {
  ...
  try {
    const jdText = String(job.description || job.title || "").slice(0, 2800);
    // Short JSON rewrite — disable thinking so reasoning_budget does not eat max_tokens.
    const result = await chat(
      "You rewrite resume bullets for a specific job. Output ONLY a JSON array of strings. Never invent employers, schools, titles, metrics, or tools not already implied by the bullets. Do not copy the input bullets verbatim.",
      `Rewrite these bullets so they emphasize overlap with this job (same facts, different emphasis/order/wording). Keep the same count. Each bullet ≤ 22 words.\n\nBullets:\n${bullets.map((b) => `- ${b}`).join("\n")}\n\nJob: ${job.title} at ${job.company}\nJD:\n${jdText}`,
      cfg,
