...
export async function evaluateTailor(cfg = loadConfig(), opts = {}) {
  ...
  } else if (readyJobs.length > 0 && doneJobs.length === readyJobs.length) {
    log(`\n[Phase 2b] All ${readyJobs.length} ready jobs already have tailored resumes — nothing to do`);
  } else if (readyJobs.length > 0) {
    log(`\n[Phase 2b] Tailoring cap (${tailorCap}) leaves ${readyJobs.length - doneJobs.length} ready jobs un-tailored`);
  }
