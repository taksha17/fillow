...
export async function trackDashboard(results = [], cfg = loadConfig()) {
  ensureTracker();
  const recorded = [];
  ...
  if (cfg.secrets.fillow_sync_url && cfg.secrets.fillow_sync_token) {
    ...
    if (cf.ok) console.log("   Cloudflare D1 sync ok");
    else if (!cf.skipped) console.warn(`   Cloudflare D1 sync failed: ${cf.status || cf.reason}`);
  }

  console.log(`📊 Tracker ${PATHS.applications}: ${metrics.total} rows`);
  console.log(`   statuses: ${JSON.stringify(metrics.byStatus)}`);
  console.log(`   dashboard: ${dash}`);
  if (replyUpdates.length) console.log(`   reply-watch updates: ${replyUpdates.length}`);
  return { recorded, metrics, dashboard: dash, replyUpdates };
}

const isCli = process.argv[1]?.endsWith("track.mjs");
if (isCli) {
  trackDashboard().catch((err) => {
    console.error(err);
    process.exit(1);
  });
}
